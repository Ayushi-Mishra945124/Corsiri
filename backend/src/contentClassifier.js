...
export function buildPrompt({
  ...
  switch (normalizedAction) {
    ...
    default:
      if (normalizedAction.startsWith("translate_")) {
        const lang = normalizedAction.replace(/^translate_(?:to_)?/, "").replace(/_/g, " ");
        const formattedLang = lang.charAt(0).toUpperCase() + lang.slice(1);
        return [
          `Translate this text accurately into ${formattedLang} while preserving the original meaning, tone, and formatting.`,
          "Provide only the translated text without extra commentary.",
          "Text:",
          text
        ].join("\n\n");
      }
