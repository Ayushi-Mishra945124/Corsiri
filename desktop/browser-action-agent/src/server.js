...
const port = Number(process.env.CORSIRI_BROWSER_AGENT_PORT || process.env.CURSIVIS_BROWSER_AGENT_PORT || 48820);
...
app.get("/health", (_req, res) => {
  res.json({
    ok: true,
    service: "corsiri-browser-action-agent",
...
async function ensureBrowserContext(preferredChannel = "") {
  ...
  launchPromise = (async () => {
    ...
    const preferredChannel = normalizedPreferred || (process.env.CORSIRI_BROWSER_CHANNEL || process.env.CURSIVIS_BROWSER_CHANNEL || "chrome").trim().toLowerCase();
    const launchTargets = [
