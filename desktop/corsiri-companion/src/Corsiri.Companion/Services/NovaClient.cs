...
public sealed class NovaClient : IDisposable
{
    ...
    public NovaClient()
    {
        var backendUrl = Environment.GetEnvironmentVariable("CORSIRI_BACKEND_URL")
            ?? Environment.GetEnvironmentVariable("CURSIVIS_BACKEND_URL")
            ?? "http://127.0.0.1:8080";
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await _httpClient.PostAsJsonAsync("/api/chat", payload, cancellationToken);
            if (response.IsSuccessStatusCode)
            {
